using LanguageLearning.Core.Application.Agreements.Queries;
...
/*public class GetAgreementById : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {

        app.MapGet("/Agreement/{id}",
            [OutputCache] async (int id, CancellationToken cancellationToken, IMediatorService _mediator, HttpContext context) =>
            {
                var result = await _mediator.Send(new GetAgreementByIdQuery(id), cancellationToken);
                return result.Match(
                    success => Results.Ok(result.Value),
                    failed => Results.Problem(detail: result.Error.Description, statusCode: StatusCodes.Status404NotFound)
                );
            })
            .Produces<Result<AgreementResponse>>()
            .ProducesProblem(StatusCodes.Status404NotFound)
            .WithDescription("GetAgreementById");
    }
    /*
